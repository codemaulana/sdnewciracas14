...
export default function NavbarPage() {
  ...
  return (
    <>
      {/* --- TOP NAVBAR (Desktop) --- 
          Diubah menjadi fixed background (bg-slate-950) dan tanpa efek scroll
      */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-slate-950 border-b border-white/5 shadow-2xl px-6 py-4">
      ...
      <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-sm">
        <m.div 
          ...
          className="bg-slate-950 border border-white/10 rounded-4xl p-2 flex items-center justify-between shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
        >
          {navLinks.map((link) => {
        ...
        {isSearchOpen && (
          ...
          >
            ...
            <div className="w-full max-w-2xl mx-auto mt-24">
              ...
              {/* Hasil Pencarian */}
              <div className="mt-10 space-y-3 max-h-[50vh] overflow-y-auto custom-scrollbar px-2 text-white">
                {results.length > 0 ? (
                  results.map((res) => (
